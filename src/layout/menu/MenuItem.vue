<template>
  <template v-if="!menu.hideMenu">
    <el-sub-menu v-if="showMenuType === 2" :index="pathResolve">
      ...
      <template v-for="(item, key) in menu.children" :key="key">
        <!-- <menu-item v-if="item.path ? (item.path.indexOf('tgt') != -1 ? true : false) : false" @click="open(item)" /> -->
        <menu-item v-if="!(item.path ? (item.path.indexOf('tgt') != -1 ? true : false) : false)" :key="key" :menu="item" :basePath="pathResolve" />
        <el-menu-item v-if="item.path ? (item.path.indexOf('tgt') != -1 ? true : false) : false" :index="item.path" @click="open(item)">{{
          item.meta.title
        }}</el-menu-item>
      </template>
    </el-sub-menu>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    function open(item) {
      ...
      let url = item.path.split('?')[0].substring(3)
      let searchURL = window.location.search //?tgt=ycccc
      console.log('url', url, searchURL)
      window.open(url + searchURL)
    }
    return {
      showMenuType,
      pathResolve,
